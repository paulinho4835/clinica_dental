...
const RULES: Record<RateLimitBucket, Rule> = {
  ...
  // Vapi reparte varios eventos por llamada (assistant-request, tool-calls,
  // end-of-call-report). Techo alto a propósito para no estrangular llamadas
  // legítimas; corta solo floods. La barrera primaria es VAPI_WEBHOOK_SECRET.
...
export function tooManyRequestsMessage(retryAfterSeconds: number): string {
  ...
  if (mins <= 1) return "Demasiadas solicitudes. Espera un momento antes de volver a intentarlo.";
