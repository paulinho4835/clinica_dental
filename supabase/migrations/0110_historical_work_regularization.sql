-- Regulariza doctor_works historicos que quedaron sin item de plan.
-- Toda decision es atomica, exclusiva del admin y queda en audit_log.
...
begin
  ...
  select * into v_work
  from doctor_works
  where id = p_work_id and patient_id = p_patient_id and clinic_id = p_clinic_id
  for update;
  if not found then
  ...
  if p_action = 'delete_duplicate' then
    if v_work.commission_paid
      or coalesce(v_work.commission_paid_amount, 0) > 0
      or v_work.staff_payment_id is not null
      or exists (select 1 from staff_payment_works where work_id = p_work_id)
    then
    ...
    values (p_clinic_id, auth.uid(), 'historical_work_deleted_duplicate', 'doctor_work', p_work_id,
      jsonb_build_object('reason', trim(p_reason), 'before', to_jsonb(v_work)));
    delete from doctor_works where id = p_work_id and clinic_id = p_clinic_id;
    return null;
  end if;
  ...
  if p_action = 'link' then
    ...
    where ti.id = p_treatment_item_id and ti.clinic_id = p_clinic_id
      and tp.clinic_id = p_clinic_id and tp.patient_id = p_patient_id
    for update of ti;
    if not found then
    ...
    select id into v_plan_id from treatment_plans
    ...
    if v_plan_id is null then
      ...
      values (p_clinic_id, p_patient_id, 'active', auth.uid()) returning id into v_plan_id;
    end if;
    select id into v_phase_id from treatment_phases
    where plan_id = v_plan_id and clinic_id = p_clinic_id
    order by phase_no limit 1 for update;
    if v_phase_id is null then
      insert into treatment_phases (clinic_id, plan_id, phase_no, title, status)
      values (p_clinic_id, v_plan_id, 1, 'Regularizacion historica', 'active') returning id into v_phase_id;
    end if;
  ...
  update doctor_works set treatment_item_id = v_item_id where id = p_work_id;
  v_payment_id := coalesce(p_payment_id, v_work.payment_id);
  if v_payment_id is not null then
    perform 1 from payments
    where id = v_payment_id and patient_id = p_patient_id and clinic_id = p_clinic_id
    ...
    update payments set treatment_item_id = v_item_id where id = v_payment_id;
  end if;

  insert into audit_log (clinic_id, actor_id, action, entity, entity_id, diff)
  values (p_clinic_id, auth.uid(), 'historical_work_' || p_action, 'doctor_work', p_work_id,
    jsonb_build_object('reason', trim(p_reason), 'before', to_jsonb(v_work),
      'treatment_item_id', v_item_id, 'payment_id', v_payment_id,
